...
import ParkingLotPickerItem from "./ParkingLotPickerItem";

const ios = Platform.OS == 'ios'

const ParkingLotPicker = () => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            >
                <TouchableOpacity
                    onPress={() => nav.goBack()}
                    style={{ padding: 10, flex: 1.5 }}>
                    <Ionicons color='white' name="arrow-back-outline" size={24} />
                </TouchableOpacity>
                <Text style={{ fontSize: 18, fontWeight: 'bold', color: "white", flex: 7 }}>Chọn bãi xe cần xem bảng giá</Text>
            </LinearGradient>
            <FlatList
                data={parkingLots}
                keyExtractor={item => `parkingLot${item.id}`}
                style={{ paddingHorizontal: 10, marginTop: 10 }}
                renderItem={({ item }) => <ParkingLotPickerItem parkingLot={item} />}
            />
        </View>
    )
}

export default ParkingLotPicker;